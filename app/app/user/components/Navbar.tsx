...
const Navbar = () => {
  ...
  const matchingServices = React.useMemo(() => {
    ...
    categories.forEach((cat: any) => {
      const catImage = cat.image_path || cat.banner_image_path || cat.icon_path ||
        "https://images.unsplash.com/photo-1563453392212-326f5e854473?w=400";
      
      list.push({
        id: `cat-${cat.id || Math.random()}`,
        name: cat.name,
      ...
      children.forEach((child: any) => {
        const childImage = child.image_path || child.icon_path || catImage;
        list.push({
          id: `child-${child.id || Math.random()}`,
          name: child.name,
          slug: cat.slug || cat.name.toLowerCase().replace(/\s+/g, "-"), // route to parent category
          category: cat.name, // parent category name
          description: child.description || `Professional ${child.name} services`,
          price: child.price || 499,
          rating: child.rating || 4.8,
          image: childImage,
          availability: "Available Now"
        });
      });
  ...
  const fetchNotifications = async () => {
    try {
      const res = await axiosClient.get("/user/notifications", { params: { limit: 20 } });
      if (res.data?.success && Array.isArray(res.data?.data)) {

...
export default function AddVendorForm({ onSuccessClose, initialValues, mode = "add", defaultRole }: AddVendorFormProps) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        console.error("Failed to load categories in AddVendorForm:", err);
      }
    };
    fetchCategories();
  }, []);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      account_level: initialValues?.account_level || "Standard Customer",
      agreeTerms: true,
    },
    ...
    onSubmit: async (values) => {
      ...
      const payload = {
        ...
        is_active: true,
        role: defaultRole || initialValues?.role || "vendor",
          ...
          <Grid container spacing={4} sx={{ mt: 10 }}>
            ...
            <Grid size={12}>
              <TextField
                ...
                SelectProps={{
                  ...
                  renderValue: (value) =>
                    renderSelectPlaceholder(
                      categories.find((c) => c.id === Number(value))?.name || value,
                      "Select category"
                    ),
                }}
                sx={{
            ...
            <Grid size={12}>
              <TextField
                ...
                variant="standard"
                label="Account Level"
                value={formik.values.account_level}
                onChange={(e) => formik.setFieldValue("account_level", e.target.value)}
                InputLabelProps={{ shrink: true }}
                SelectProps={{
                  displayEmpty: true,
                  renderValue: (value) => renderSelectPlaceholder(value, "Select Account Level"),
                }}
                sx={{
                  "& .MuiInputLabel-root": {
                    color: "#9CA3AF",
              ...
              >
                <MenuItem value="Standard Customer">Standard Customer</MenuItem>

...
const CartPage = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (addresses.length === 0) {
      toast.error("Please add a saved address in your Profile page before checking out.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        notes: "Booking placed from web cart."
      };
      
      const res = await axiosClient.post("/bookings/checkout", payload);
      if (res.data?.success) {
        setShowSuccess(true);
      } else {
        toast.error(res.data?.message || "Failed to place bookings");
      }
    } catch (err: any) {
              ...
              >
                ...
                <Box sx={{ p: 3, borderBottom: "1px solid #E3E3E3" }}>
                  <Box sx={{ display: "flex", gap: 2 }}>
                    ...
                    <Button
                      variant="contained"
                      onClick={() => toast.info("Promo Codes feature is coming soon!")}
                      sx={{
                        bgcolor: "#6E42E5",
                        color: "white",
                        px: 3,
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ fontSize: "14px", color: "#6B7280" }}>
            You must be logged in to proceed with checkout. Please log in to complete your booking.
          </DialogContentText>
        </DialogContent>

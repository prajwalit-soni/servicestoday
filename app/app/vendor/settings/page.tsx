...
export default function VendorSettings() {
    ...
    axiosClient.get("/public/categories")
      .then((res) => {
        if (res.data && res.data.success) {
          ...
          // Pre-select first category if categoryId is empty
          setFormData((prev) => {
            if (!prev.categoryId && flat.length > 0) {
              return { ...prev, categoryId: flat[0].id };
            }
            return prev;
          });
        }
      ...
      .then((res) => {
        if (res.data && res.data.success && res.data.data) {
          ...
          setFormData((prev) => ({
            ...
            latitude: data.latitude ? String(data.latitude) : "22.3597",
            longitude: data.longitude ? String(data.longitude) : "82.7501",
          ...
          if (data.profile_image_path) setProfileImagePreview(data.profile_image_path);
          if (data.shop_image_path) setShopImagePreview(data.shop_image_path);
        }
      })
      .catch((err) => console.error("Error fetching provider profile:", err));
  }, []);

  // Fetch available services dynamically when categoryId changes
  useEffect(() => {
    ...
    axiosClient.get("/public/services", { params: { category_id: formData.categoryId, limit: 100 } })
      .then((res) => {
        if (res.data && res.data.success && res.data.data) {
          const titles: string[] = Array.from(new Set(res.data.data.map((s: any) => s.title)));
          if (titles.length > 0) {
            setAvailableServices(titles);
            setFormData((prev) => {
              // Pre-select first service if none are selected
              if (prev.servicesOffered.length === 0) {
                return { ...prev, servicesOffered: [titles[0]] };
              }
              return prev;
            });
            return;
        ...
        // Fallback services if none are fetched
        const categoryObj = categories.find((c) => String(c.id) === String(formData.categoryId));
        const categoryName = categoryObj ? categoryObj.name : "";
        const fallbackMap: Record<string, string[]> = {
          "Plumber": ["Tap Repair & Installation", "Drain Blockage Clearance", "Water Tank Cleaning", "Pipe Leakage Repair"],
          "Electrician": ["Fan Repair & Installation", "House Wiring & Repair", "Switchboard Fitting", "Light Installation"],
          "Beautician": ["Facial & Skincare", "Manicure & Pedicure", "Waxing", "Hair Styling"],
          "AC Repair": ["AC Installation", "AC Servicing", "AC Gas Charging", "AC Repair"],
          "Carpenter": ["Furniture Repair", "Door Lock Fitting", "Modular Kitchen Woodwork", "Sofa Repair"],
          "Cleaning": ["Bathroom Deep Cleaning", "Kitchen Deep Cleaning", "Sofa Cleaning", "Full House Deep Cleaning"],
        };
        const fallbacks = fallbackMap[categoryName] || [
          categoryName ? `${categoryName} General Service` : "General Service",
          categoryName ? `${categoryName} Consultation` : "Consultation"
        ];
        setAvailableServices(fallbacks);
        setFormData((prev) => {
          if (prev.servicesOffered.length === 0) {
            return { ...prev, servicesOffered: [fallbacks[0]] };
          }
          return prev;
        });
      })
      .catch((err) => {
        ...
        setAvailableServices(["General Service"]);
      });
  ...
  const handleFileChange = (field: string, file: File | null) => {
    if (file) {
      handleInputChange(field + "FileName", file.name);
      if (field === "pan") setPanFile(file);
      if (field === "aadhar") setAadharFile(file);
      if (field === "reg") setRegFile(file);
      if (field === "gst") setGstFile(file);
    } else {
      handleInputChange(field + "FileName", "");
      if (field === "pan") setPanFile(null);
      if (field === "aadhar") setAadharFile(null);
      if (field === "reg") setRegFile(null);
      if (field === "gst") setGstFile(null);
    }
  };
  ...
  const handleSubmitConfirm = async () => {
    ...
    try {
      ...
      if (formData.categoryId) data.append("category_id", String(formData.categoryId));
      ...
      const aboutText = `Operational in ${formData.primaryCity}. Call-out fee: ₹${formData.baseCallOutFee}. Standard hourly rate: ₹${formData.hourlyRate}. Min order: ₹${formData.minOrderValue}.`;
      data.append("about", aboutText);
      
      if (formData.bankName) data.append("bank_name", formData.bankName);
      if (formData.branchName) data.append("branch_name", formData.branchName);
      if (formData.accountHolder) data.append("account_holder", formData.accountHolder);
      if (formData.accountNumber) data.append("account_number", formData.accountNumber);
      if (formData.ifscCode) data.append("ifsc_code", formData.ifscCode);
      if (formData.accountType) data.append("account_type", formData.accountType);
      
      // Default why_choose_us points
      const whyChooseUsData = [
        { "title": "Verified Experts", "subtitle": "100% background checked and highly rated service professionals.", "icon": "PeopleIcon" },
        { "title": "Clear Payouts & Pricing", "subtitle": "Upfront quotes without any hidden transactional fees.", "icon": "VerifiedIcon" },
        { "title": "24/7 Premium Support", "subtitle": "Dedicated manager assigned for client resolution help.", "icon": "BusinessIcon" }
      ];
      data.append("why_choose_us", JSON.stringify(whyChooseUsData));

      if (formData.supportPhone) data.append("phone", formData.supportPhone);
      if (formData.supportEmail) data.append("email", formData.supportEmail);
      if (formData.streetAddress) data.append("address", formData.streetAddress);

      if (profileFile) data.append("profile_image", profileFile);
      if (shopFile) data.append("shop_image", shopFile);

      // Services offered mapping
      const mappedServices = formData.servicesOffered.map((name) => ({
        name,
        price: formData.hourlyRate || 500,
        description: "Professional service package"
      }));
      data.append("services", JSON.stringify(mappedServices));
      ...
      data.append("availabilities", JSON.stringify(defaultAvailabilities));

      let response;
      if (profileExists) {
        response = await axiosClient.put("/provider/profile", data, {
          headers: { "Content-Type": "multipart/form-data" }
        });
      } else {
        response = await axiosClient.post("/provider/profile", data, {
          headers: { "Content-Type": "multipart/form-data" }
        });
      }
      ...
      if (response.data && response.data.success) {
        const uploadDoc = async (type: string, file: File) => {
          ...
          docData.append("document_type", type);
          docData.append("file", file);
          await axiosClient.post("/provider/documents", docData, {
            headers: { "Content-Type": "multipart/form-data" }
          });
        };

        if (panFile) await uploadDoc("PAN Card", panFile);
        if (aadharFile) await uploadDoc("Aadhar Card", aadharFile);
        if (regFile) await uploadDoc("Business Reg Certificate", regFile);
        if (gstFile) await uploadDoc("GST Certificate", gstFile);
        
        setProfileExists(true);
        setIsSuccess(true);
      }
    } catch (err) {
      console.error("Error saving provider profile settings:", err);
    } finally {
      setSaving(false);
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            {activeStep === 0 && (
              <Grid container spacing={3}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <Box sx={{ border: "1px dashed #D1D5DB", p: 2, borderRadius: "12px", textAlign: "center", bgcolor: "#FAFAFA" }}>
                    <Typography variant="subtitle2" fontWeight={600} color="#374151" sx={{ mb: 1 }}>Business Logo</Typography>
                    {profileImagePreview ? (
                      <Box sx={{ position: "relative", display: "inline-block" }}>
                        <img src={profileImagePreview} alt="Logo Preview" style={{ width: 60, height: 60, borderRadius: "50%", objectFit: "cover" }} />
                        <IconButton size="small" onClick={() => { setProfileFile(null); setProfileImagePreview(null); }} sx={{ position: "absolute", top: -5, right: -5, bgcolor: "rgba(255,255,255,0.8)" }}>
                          <CloseIcon sx={{ fontSize: 12 }} />
                        </IconButton>
                      </Box>
                    ) : (
                      <Button variant="outlined" size="small" component="label" sx={{ textTransform: "none", color: "#635BFF", borderColor: "#635BFF" }}>
                        Upload Logo
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <Box sx={{ border: "1px dashed #D1D5DB", p: 2, borderRadius: "12px", textAlign: "center", bgcolor: "#FAFAFA" }}>
                    <Typography variant="subtitle2" fontWeight={600} color="#374151" sx={{ mb: 1 }}>Cover Banner</Typography>
                    {shopImagePreview ? (
                      <Box sx={{ position: "relative", display: "inline-block" }}>
                        <img src={shopImagePreview} alt="Cover Preview" style={{ width: 120, height: 60, borderRadius: "8px", objectFit: "cover" }} />
                        <IconButton size="small" onClick={() => { setShopFile(null); setShopImagePreview(null); }} sx={{ position: "absolute", top: -5, right: -5, bgcolor: "rgba(255,255,255,0.8)" }}>
                          <CloseIcon sx={{ fontSize: 12 }} />
                        </IconButton>
                      </Box>
                    ) : (
                      <Button variant="outlined" size="small" component="label" sx={{ textTransform: "none", color: "#635BFF", borderColor: "#635BFF" }}>
                        Upload Cover
                        <input type="file" accept="image/*" hidden onChange={handleShopImageChange} />
                      </Button>
                    )}
                  </Box>
                </Grid>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <TextField
                    fullWidth
                    label="Official Business / Provider Name"
                    variant="outlined"
                    required
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <FormControl fullWidth variant="outlined" required error={Boolean(errors.categoryId)}>
                  ...
                  <Box sx={{ border: errors.panFileName ? "2px dashed #EF4444" : "1px dashed #D1D5DB", p: 3, borderRadius: "16px", bgcolor: "#FAFAFA", textAlign: "center" }}>
                    ...
                    {formData.panFileName ? (
                      <Box sx={{ display: "inline-flex", alignItems: "center", bgcolor: "#E6F4EA", px: 2, py: 0.5, borderRadius: "20px", mb: 2 }}>
                        ...
                        <IconButton size="small" onClick={() => handleFileChange("pan", null)} sx={{ ml: 1, p: 0.2 }}><CloseIcon sx={{ fontSize: 14 }} /></IconButton>
                      </Box>
                    ) : (
                      <Button variant="outlined" size="small" component="label" sx={{ textTransform: "none", color: "#635BFF", borderColor: "#635BFF" }}>
                        Browse File
                        <input type="file" accept=".pdf,image/*" hidden onChange={(e) => handleFileChange("pan", e.target.files ? e.target.files[0] : null)} />
                      </Button>
                    )}
                  </Box>
                </Grid>
                  ...
                  <Box sx={{ border: errors.aadharFileName ? "2px dashed #EF4444" : "1px dashed #D1D5DB", p: 3, borderRadius: "16px", bgcolor: "#FAFAFA", textAlign: "center" }}>
                    ...
                    {formData.aadharFileName ? (
                      <Box sx={{ display: "inline-flex", alignItems: "center", bgcolor: "#E6F4EA", px: 2, py: 0.5, borderRadius: "20px", mb: 2 }}>
                        ...
                        <IconButton size="small" onClick={() => handleFileChange("aadhar", null)} sx={{ ml: 1, p: 0.2 }}><CloseIcon sx={{ fontSize: 14 }} /></IconButton>
                      </Box>
                    ) : (
                      <Button variant="outlined" size="small" component="label" sx={{ textTransform: "none", color: "#635BFF", borderColor: "#635BFF" }}>
                        Browse File
                        <input type="file" accept=".pdf,image/*" hidden onChange={(e) => handleFileChange("aadhar", e.target.files ? e.target.files[0] : null)} />
                      </Button>
                    )}
                  </Box>
                </Grid>
                  ...
                  <Box sx={{ border: "1px dashed #D1D5DB", p: 3, borderRadius: "16px", bgcolor: "#FAFAFA", textAlign: "center" }}>
                    ...
                    {formData.regFileName ? (
                      <Box sx={{ display: "inline-flex", alignItems: "center", bgcolor: "#E6F4EA", px: 2, py: 0.5, borderRadius: "20px", mb: 2 }}>
                        ...
                        <IconButton size="small" onClick={() => handleFileChange("reg", null)} sx={{ ml: 1, p: 0.2 }}><CloseIcon sx={{ fontSize: 14 }} /></IconButton>
                      </Box>
                    ) : (
                      <Button variant="outlined" size="small" component="label" sx={{ textTransform: "none", color: "#635BFF", borderColor: "#635BFF" }}>
                        Browse File
                        <input type="file" accept=".pdf,image/*" hidden onChange={(e) => handleFileChange("reg", e.target.files ? e.target.files[0] : null)} />
                      </Button>
                    )}
                  </Box>
                </Grid>
                  ...
                  <Box sx={{ border: "1px dashed #D1D5DB", p: 3, borderRadius: "16px", bgcolor: "#FAFAFA", textAlign: "center" }}>
                    ...
                    {formData.gstFileName ? (
                      <Box sx={{ display: "inline-flex", alignItems: "center", bgcolor: "#E6F4EA", px: 2, py: 0.5, borderRadius: "20px", mb: 2 }}>
                        ...
                        <IconButton size="small" onClick={() => handleFileChange("gst", null)} sx={{ ml: 1, p: 0.2 }}><CloseIcon sx={{ fontSize: 14 }} /></IconButton>
                      </Box>
                    ) : (
                      <Button variant="outlined" size="small" component="label" sx={{ textTransform: "none", color: "#635BFF", borderColor: "#635BFF" }}>
                        Browse File
                        <input type="file" accept=".pdf,image/*" hidden onChange={(e) => handleFileChange("gst", e.target.files ? e.target.files[0] : null)} />
                      </Button>
                    )}
                  </Box>
                </Grid>

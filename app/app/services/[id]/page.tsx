...
const ServiceDetailPage = () => {
  ...
  const mapApiCategoryToServiceDetail = (apiData: any) => {
    ...
    const mappedPackages = (apiData.subcategories || []).map((sub: any) => {
      let parsedSections = [];
      if (sub.description_sections) {
        try {
          parsedSections = typeof sub.description_sections === 'string'
            ? JSON.parse(sub.description_sections)
            : sub.description_sections;
        } catch (e) {
          console.error("Error parsing subcategory description sections:", e);
        }
      }
      ...
      return {
        ...
        price: sub.discount_price || sub.price || 499,
        originalPrice: sub.discount_price ? sub.price : undefined,
